...
const SECTIONS = [
  ...
  },
  {
    ...
    body: 'This system is exclusively for authorized students, faculty, and staff of the institution. Your account is personal and non-transferable. You are responsible for all activity under your account.',
  },
  {
  ...
  },
  {
    ...
    body: 'We collect and process your personal information (name, email, vehicle details) solely for parking management purposes. Your data will not be shared with third parties without your consent, except as required by law.',
  },
  {
  ...
  return (
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <View style={{ paddingHorizontal: 24, paddingTop: 24, paddingBottom: 16, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        <View style={{ backgroundColor: t.primaryLight, padding: 10, borderRadius: 12 }}>
          <ShieldCheck color={t.primary} size={22} strokeWidth={1.5} />
        </View>
        <View>
          <Text style={{ fontFamily: 'Poppins700', fontSize: 18, color: t.text }}>Terms & Conditions</Text>
          <Text style={{ fontFamily: 'Poppins400', fontSize: 12, color: t.textMuted }}>Please read carefully before continuing</Text>
        </View>
      </View>
      ...
      >
        <View style={{ backgroundColor: t.card, borderRadius: 16, borderWidth: 1, borderColor: t.cardBorder, padding: 20, marginBottom: 16 }}>
          <Text style={{ fontFamily: 'Poppins400', fontSize: 13, color: t.textMuted, lineHeight: 20, marginBottom: 16 }}>
            Welcome to the School Parking System. These terms govern your use of the application. Scroll to the bottom to accept.
          </Text>
          {SECTIONS.map((s, i) => (
            <View key={i} style={{ marginBottom: i < SECTIONS.length - 1 ? 20 : 0 }}>
              <Text style={{ fontFamily: 'Poppins600', fontSize: 13, color: t.text, marginBottom: 6 }}>{s.title}</Text>
              <Text style={{ fontFamily: 'Poppins400', fontSize: 13, color: t.textMuted, lineHeight: 20 }}>{s.body}</Text>
            </View>
          ))}
        </View>

        {!scrolledToBottom && (
          <Text style={{ textAlign: 'center', fontFamily: 'Poppins400', fontSize: 12, color: t.textFaint, marginBottom: 16 }}>
      ...
      {/* Accept button */}
      <View style={{ paddingHorizontal: 24, paddingTop: 12, paddingBottom: 8, borderTopWidth: 1, borderTopColor: t.cardBorder }}>
        <Pressable
          onPress={handleAccept}
          disabled={!scrolledToBottom || loading}
          style={{
            backgroundColor: scrolledToBottom && !loading ? t.primary : t.cardBorder,
            borderRadius: 14,
            paddingVertical: 16,
            alignItems: 'center',
        ...
        >
          <Text style={{ fontFamily: 'Poppins600', fontSize: 15, color: scrolledToBottom && !loading ? '#fff' : t.textFaint }}>
