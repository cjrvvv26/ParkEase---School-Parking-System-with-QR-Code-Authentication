...
import { ChevronLeft, MapPin, Clock, ParkingSquare, Navigation, ParkingCircle } from 'lucide-react-native';
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: t.bg }}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        {!loading && !haveSlot ? (
          /* Not in-slot empty state */
          <View style={{ marginHorizontal: 20, marginTop: 24, alignItems: 'center', backgroundColor: t.card, borderRadius: 24, borderWidth: 1, borderColor: t.cardBorder, padding: 36, gap: 16 }}>
            <View style={{ backgroundColor: t.primaryLight, padding: 20, borderRadius: 24 }}>
              <ParkingCircle color={t.primary} size={40} />
            </View>
            <View style={{ alignItems: 'center', gap: 6 }}>
              <Text style={{ fontFamily: 'Poppins700', fontSize: 16, color: t.text }}>No Active Parking</Text>
              <Text style={{ fontFamily: 'Poppins400', fontSize: 13, color: t.textFaint, textAlign: 'center', lineHeight: 20 }}>
                You're not currently occupying any parking slot. Scan a slot QR code to get started.
              </Text>
            </View>
            <Pressable
              onPress={() => router.push('/Scan')}
              style={{ backgroundColor: t.primary, paddingHorizontal: 28, paddingVertical: 12, borderRadius: 14, marginTop: 4 }}
            >
              <Text style={{ fontFamily: 'Poppins600', fontSize: 13, color: '#fff' }}>Scan a Slot</Text>
            </Pressable>
          </View>
        ) : (
          <>
