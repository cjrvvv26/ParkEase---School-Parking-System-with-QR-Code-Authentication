...
export default function ResetPassword() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!fp_email || !fp_token) { router.replace('/Recovery'); return; }
      setEmail(fp_email);
  ...
  const handleReset = async () => {
    ...
    setError('');
    try {
      await api.patch('auth/forgot-password/reset', { email, resetToken, password });
      await AsyncStorage.removeItem('fp_email');
      await AsyncStorage.removeItem('fp_reset_token');
      router.replace({ pathname: '/SignIn', params: { message: 'Password reset successfully! Please sign in.' } });
    } catch (e) {
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps='handled' showsVerticalScrollIndicator={false}>
          ...
          <View style={{ alignItems: 'center', justifyContent: 'center', paddingVertical: 48, gap: 12 }}>
            ...
            <Text style={{ fontFamily: 'Poppins700', fontSize: 26, color: '#fff', letterSpacing: 0.3 }}>Reset Password</Text>
          ...
          <View style={{ flex: 1, backgroundColor: t.card, borderTopLeftRadius: 32, borderTopRightRadius: 32, paddingHorizontal: 28, paddingTop: 36, paddingBottom: 40 }}>
            ...
            <Text style={{ fontFamily: 'Poppins700', fontSize: 22, color: t.text, marginBottom: 4 }}>New Password</Text>
            <Text style={{ fontFamily: 'Poppins400', fontSize: 13, color: t.textMuted, marginBottom: 28 }}>
              Make sure it's something you'll remember
            </Text>

            {/* New password */}
            <Text style={{ fontFamily: 'Poppins600', fontSize: 13, color: t.text, marginBottom: 8 }}>New Password</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: t.inputBg, borderRadius: 14, borderWidth: 1, borderColor: t.cardBorder, paddingHorizontal: 14, gap: 10, marginBottom: 14 }}>
              <TextInput
                value={password}
              ...
              />
              <Pressable onPress={() => setShowPassword((p) => ({ ...p, new: !p.new }))} hitSlop={8}>
                {showPassword.new
            ...
            {(touched || password.length > 0) && (
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 }}>
                {RULES.map((r) => {
                  ...
                  return (
                    <View key={r.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, width: '47%' }}>
                      {pass
                        ? <Check color='#16a34a' size={13} />
                        : <X color='#9ca3af' size={13} />}
                      <Text style={{ fontFamily: 'Poppins400', fontSize: 11, color: pass ? '#16a34a' : '#9ca3af', flex: 1 }}>
                        {r.label}
                      </Text>
                    </View>
                  );
            ...
            {/* Confirm password */}
            <Text style={{ fontFamily: 'Poppins600', fontSize: 13, color: t.text, marginBottom: 8 }}>Confirm Password</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: confirmBg(), borderRadius: 14, borderWidth: 1, borderColor: confirmBorderColor(), paddingHorizontal: 14, gap: 10, marginBottom: 6 }}>
              <TextInput
                value={confirm}
                ...
                style={{ flex: 1, fontFamily: 'Poppins400', fontSize: 14, color: t.text, paddingVertical: 14 }}
              />
              <Pressable onPress={() => setShowPassword((p) => ({ ...p, confirm: !p.confirm }))} hitSlop={8}>
            ...
            {touched && confirm !== '' && (
              <Text style={{ fontFamily: 'Poppins400', fontSize: 12, color: passwordsMatch ? '#16a34a' : '#ef4444', marginBottom: 8 }}>
            ...
            <Pressable
              ...
              style={{
                ...
                shadowOpacity: canSubmit ? 0.3 : 0, shadowRadius: 8,
                elevation: canSubmit ? 6 : 0,
              }}
            >
              <Text style={{ fontFamily: 'Poppins600', fontSize: 15, color: canSubmit ? '#fff' : '#9ca3af' }}>
