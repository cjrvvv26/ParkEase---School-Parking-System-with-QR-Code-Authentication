...
function ConfirmDialog({
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40'>
      <div
        className={`rounded-2xl shadow-2xl w-[380px] p-6 flex flex-col gap-4 ${dark ? 'bg-[#2f2f2f]' : 'bg-white'}`}
      >
        ...
        <div className='flex gap-3 mt-1'>
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className={`flex-1 py-2 rounded-xl text-sm text-white transition ${confirmClass} disabled:opacity-60`}
          >
...
export default function SlotManagement({
  ...
  const slotStatus = selectedShape?.assignedStudentId
    ? 'Exclusive'
  ...
  return (
    <div className='flex flex-col w-[620px] gap-0 text-sm select-none'>
      {/* Header */}
      <div
        className={`flex items-center justify-between px-6 pt-6 pb-4 border-b ${border}`}
      >
        <div className='flex items-center gap-3'>
          ...
          <div>
            <h1
              className={`font-semibold text-base ${dark ? 'text-gray-200' : 'text-gray-800'}`}
            >
              Manage Slot
            </h1>
        ...
        <div className='flex items-center gap-2'>
          <span
            className={`text-xs font-medium px-3 py-1 rounded-full ${statusColor[slotStatus]}`}
          >
            {slotStatus}
          </span>
          <button
      ...
      <div className={`px-6 py-4 border-b ${border}`}>
        ...
        {assignedUser ? (
          <div
            className={`flex items-center justify-between rounded-xl p-3 border ${dark ? 'bg-[#3a3a3a] border-[#4a4a4a]' : 'bg-blue-50 border-blue-100'}`}
          >
            <div className='flex items-center gap-3'>
              <Avatar
                src={assignedUser.profileDetails?.url}
                name={assignedUser.name?.firstName}
              />
              <div>
                <p
                  className={`font-medium ${dark ? 'text-gray-200' : 'text-gray-800'}`}
                >
            ...
            <div className='flex items-center gap-2'>
              <span
                className={`text-xs border px-2 py-1 rounded-lg capitalize ${dark ? 'bg-[#2f2f2f] border-[#4a4a4a] text-blue-400' : 'bg-white border-blue-200 text-blue-500'}`}
              >
                {assignedUser.role}
              </span>
              <button
                onClick={() => setConfirm({ type: 'remove' })}
                className='flex items-center gap-1.5 text-xs bg-rose-500 hover:bg-rose-600 text-white px-3 py-1.5 rounded-lg transition'
              >
          ...
          <div
            className={`flex items-center gap-3 rounded-xl p-3 border border-dashed ${dark ? 'bg-[#3a3a3a] border-[#4a4a4a]' : 'bg-gray-50 border-gray-200'}`}
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${dark ? 'bg-[#2f2f2f]' : 'bg-gray-100'}`}
            >
              <UserX size={16} className='text-gray-300' />
            </div>
            <p className='text-gray-400 text-xs'>
              No user assigned to this slot
            </p>
          </div>
        )}
      </div>
      ...
      <div className='px-6 py-4 flex flex-col gap-3'>
        ...
        <div className='flex flex-col max-h-[280px] overflow-y-auto gap-1 pr-1'>
          {filtered.length === 0 ? (
            ...
            filtered.map((user, i) => {
              const firstName =
                user.profile?.name?.firstName || user.name?.firstName || '';
              const lastName =
                user.profile?.name?.lastName || user.name?.lastName || '';
              return (
                <div
                  key={i}
                  className={`flex items-center justify-between p-3 rounded-xl border border-transparent transition group ${dark ? 'hover:bg-[#3a3a3a] hover:border-[#4a4a4a]' : 'hover:bg-gray-50 hover:border-gray-100'}`}
                >
                  <div className='flex items-center gap-3'>
                  ...
                  <div className='flex items-center gap-2'>
                    <span
                      className={`text-xs px-2 py-1 rounded-lg capitalize ${dark ? 'bg-[#2f2f2f] text-gray-400' : 'bg-gray-100 text-gray-400'}`}
                    >
                      {user.role}
                    </span>
                    <button
