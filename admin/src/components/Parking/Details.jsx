...
import { Settings, Building2, Info, Clock } from 'lucide-react';
...
export default function Details({ selectedShape, loading }) {
  ...
  return (
    <section
      className={`w-[380px] border-l flex flex-col ${border} ${!updatedSlot ? 'items-center justify-center' : ''} p-5 gap-4`}
    >
      {updatedSlot ? (
          ...
          <div className='flex flex-col gap-4 w-full'>
            {updatedSlot.metadata?.type === 'building' && (
              <div className='flex flex-col gap-3'>
                ...
                <div
                  className={`flex justify-between text-xs text-gray-500 border-t pt-3 ${border}`}
                >
                  <span className='text-gray-400'>Label</span>
                  <span
                    className={`font-medium ${dark ? 'text-gray-300' : ''}`}
                  >
                    {updatedSlot.metadata?.label}
                  </span>
                </div>
            ...
            {updatedSlot.metadata?.type === 'slot' && (
              <div className='flex flex-col gap-4'>
                ...
                {updatedSlot.assignedStudentId ? (
                  <div className='flex flex-col gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100'>
                    <div className='flex gap-3 items-center'>
                      ...
                      <div className='flex flex-col gap-0.5 overflow-hidden'>
                        ...
                        <p className='text-xs text-gray-400'>
                          {updatedSlot.studentNo}
                        </p>
                        <div className='flex gap-1 flex-wrap mt-1'>
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${updatedSlot.status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}
                          >
                            {updatedSlot.status?.charAt(0).toUpperCase() +
                              updatedSlot.status?.slice(1)}
                          </span>
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${updatedSlot.payment?.isPaid ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-500'}`}
                          >
                  ...
                  <div
                    className={`h-28 rounded-xl border border-dashed flex flex-col items-center justify-center gap-1 text-gray-300 ${dark ? 'bg-[#3a3a3a] border-[#4a4a4a]' : 'bg-gray-50 border-gray-200'}`}
                  >
                ...
                <div className='flex flex-col gap-2 text-xs'>
                  ...
                  <div className='flex justify-between text-gray-500'>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[statusLabel]}`}
                    >
                      {statusLabel}
                    </span>
                  </div>
        ...
        <div className='flex flex-col gap-2 items-center text-center'>
          <div
            className={`p-3 rounded-full ${dark ? 'bg-[#3a3a3a]' : 'bg-gray-100'}`}
          >
