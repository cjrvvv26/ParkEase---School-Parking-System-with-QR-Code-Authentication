...
export default function Logs() {
  ...
  useEffect(() => {
    const getLogs = async () => {
      const data = await fetchData(`/activity?page=${page}&limit=${limit}&actionType=${actionType}`);
      setLogs(data.logs);
      setTotal(data.total);
    };
    getLogs();
    ...
    <>
      <div className='flex justify-between items-center pt-5 px-5'>
        <div className='flex flex-col'>
          ...
          <p className='text-gray-400'>See every activity occurring within the system</p>
        </div>
        <div className='flex gap-5 items-center text-gray-400'>
          <p className='text-sm'>{logs.length > 0 ? (page - 1) * limit + 1 : 0} – {(page - 1) * limit + logs.length} of {total}</p>
          <div className='flex gap-3'>
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='size-4 cursor-pointer hover:text-gray-700' onClick={() => { if (page > 1) setPage((p) => p - 1); }}>
              <path strokeLinecap='round' strokeLinejoin='round' d='M15.75 19.5 8.25 12l7.5-7.5' />
            </svg>
            <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='size-4 cursor-pointer hover:text-gray-700' onClick={() => { if ((page - 1) * limit + logs.length !== total) setPage((p) => p + 1); }}>
      ...
      <div className='flex flex-1 flex-col'>
        ...
        {logs.length < 1 && !loading && <p className='text-center mt-36 text-gray-400'>No records found</p>}

        {!loading && logs && logs.map((log) => (
          <div key={log._id} className={`py-4 px-2 w-full grid grid-cols-[230px_2fr_auto] gap-3 ${dark ? 'hover:bg-[#2f2f2f]' : 'hover:bg-gray-100'}`}>
            <div className='flex gap-3 items-center'>
              <img src={log.userId?.profileDetails?.url} alt='' className='h-12 w-12 object-cover rounded-full' />
              <p className='font-medium text-sm overflow-hidden text-nowrap w-[120px]'>{log.userFullName}</p>
            </div>
            <div className='flex items-center text-left overflow-hidden'>
              <p className={`truncate ${dark ? 'text-gray-300' : 'text-gray-700'}`}>{log.description}</p>
            </div>
            <p className='items-center flex justify-center text-xs font-medium text-gray-400'>
