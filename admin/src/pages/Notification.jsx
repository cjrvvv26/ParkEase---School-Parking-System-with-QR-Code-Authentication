    ...
    <>
      ...
      <section className='flex-1 items-center gap-5 mt-5 flex flex-col'>
        <header className='max-w-3xl w-full flex justify-between items-center'>
          <div className='bg-gray-100 *:py-2 *:hover:bg-violet-500 *:hover:text-white text-gray-600 *:rounded-md text-sm *:px-5 rounded-md p-2 flex gap-3'>
            {['all', 'read', 'unread'].map((option) => (
              <button
                ...
                className={`${type === option && 'bg-violet-500 text-white'}`}
              >
          ...
          <div className='flex text-xs gap-5 text-gray-400 mr-5 items-center'>
            <p>
              {(page - 1) * 10 + 1} - {(page - 1) * 10 + notifications.length}{' '}
              of {total}
            </p>
