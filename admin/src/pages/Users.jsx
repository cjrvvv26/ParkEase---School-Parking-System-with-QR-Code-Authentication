    ...
    <>
      <header className='flex justify-between px-5 pt-5 items-center'>
        ...
        <div className='flex gap-5 items-center'>
          <Link
            to='/add-student'
            className='p-4 bg-blue-500 hover:bg-blue-600 text-white flex items-center gap-2 rounded-full font-medium'
          >
            <svg
          ...
          <div className='relative flex items-center justify-center'>
            ...
            {showFilter && (
              <div
                className={`absolute z-10 top-12 right-0 rounded-lg border ${border} p-5 flex flex-col gap-3 text-xs ${dark ? 'bg-[#2f2f2f]' : 'bg-white'}`}
              >
                <div className='flex justify-between items-center'>
                  <h2 className='text-sm font-medium'>Filter by</h2>
                  <button
                    onClick={() => setFilter({ role: 'all', status: 'all' })}
                    ...
                    <div className='flex gap-2'>
                      {['All', 'Student', 'Faculty', 'Guard'].map((role, i) => (
                        <button
                          ...
                          className={`${role.toLowerCase() === filter.role ? 'bg-blue-500 text-white' : 'text-blue-500 bg-blue-100 border-blue-500 hover:bg-blue-500 hover:text-white'} py-2 px-4 rounded-md border text-nowrap`}
                        >
                          {role}
                        </button>
                      ))}
      ...
      <div className='h-[calc(100vh-217.6px)] mb-3'>
        <table
          className={`flex h-full flex-col mx-3 border ${border} rounded-xl overflow-hidden`}
        >
              ...
              <div className='flex gap-5'>
                ...
                <div className='flex gap-3'>
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke='currentColor'
                    className='size-4 hover:text-gray-700 text-gray-400 cursor-pointer'
                    onClick={() => {
                  ...
                  <svg
                    ...
                    className='size-4 hover:text-gray-700 text-gray-400 cursor-pointer'
                    onClick={() => {
                      if (count.current < count.total) setPage((p) => p + 1);
                    }}
                  >
                    <path
                      strokeLinecap='round'
              ...
              Array.from({ length: 4 }, (_, i) => (
                ...
                >
                  <div className='flex gap-2 items-center'>
                    <span
                      className={`min-h-12 min-w-12 rounded-full ${dark ? 'bg-[#3a3a3a]' : 'bg-gray-100'}`}
                    ></span>
                    <div className='flex flex-col w-full gap-1'>
                      <span
                        className={`py-2 rounded-full h-1 ${dark ? 'bg-[#3a3a3a]' : 'bg-gray-100'}`}
                      ></span>
                      <span
                        className={`py-2 rounded-full w-[70%] h-1 ${dark ? 'bg-[#3a3a3a]' : 'bg-gray-100'}`}
                      ></span>
                    </div>
                  </div>
                  ...
                  <span
                    className={`py-4 rounded-md w-10 h-1 ${dark ? 'bg-[#3a3a3a]' : 'bg-gray-100'}`}
                  ></span>
                </tr>
              ))
            ) : users ? (
