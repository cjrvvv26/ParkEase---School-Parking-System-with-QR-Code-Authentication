...
export default function Chats() {
  ...
  return (
    <div className={`flex h-[calc(100vh-104px)] ${dark ? 'bg-[#1a1a1a]' : 'bg-gray-100'}`}>
      {/* Sidebar */}
      <div className={`w-80 border-r ${border} flex flex-col ${dark ? 'bg-[#242424]' : 'bg-white'}`}>
        <div className={`p-4 border-b ${border}`}>
          <h1 className={`text-2xl font-bold mb-4 ${dark ? 'text-gray-100' : 'text-gray-700'}`}>Chats</h1>
              ...
              return (
                <div
                  ...
                  className={`p-4 border-b cursor-pointer transition ${border} ${
                    selected?._id === u._id ? (dark ? 'bg-violet-500/10' : 'bg-violet-50') : (dark ? 'hover:bg-[#2f2f2f]' : 'hover:bg-gray-50')
                  }`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-sm truncate ${dark ? 'text-gray-200' : 'text-gray-700'}`}>
                        {formatName(u.name)}
                      </p>
                      {lastMsg ? (
      ...
      {/* Chat Area */}
      <div className={`flex-1 flex flex-col ${dark ? 'bg-[#242424]' : 'bg-white'}`}>
        {selected ? (
          <>
            {/* Header */}
            <div className={`border-b p-4 shadow-sm flex items-center gap-3 ${border} ${dark ? 'bg-[#242424]' : 'bg-white'}`}>
              ...
              <div>
                <p className={`font-semibold ${dark ? 'text-gray-100' : 'text-gray-700'}`}>{formatName(selected.name)}</p>
                <p className="text-xs text-gray-400 capitalize">{selected.role}</p>
              </div>
            </div>

            {/* Messages */}
            <div className={`flex-1 overflow-y-auto p-4 space-y-3 ${dark ? 'bg-[#1a1a1a]' : 'bg-gray-50'}`}>
              {messages.map((msg, i) => {
                    ...
                    <div
                      className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl text-sm ${
                        isOwn
                          ...
                          : dark ? 'bg-[#2f2f2f] text-gray-200 rounded-bl-none' : 'bg-gray-200 text-gray-900 rounded-bl-none'
                      }`}
                    >
                      <p>{msg.message}</p>
            ...
            {/* Input */}
            <div className={`border-t p-4 ${border} ${dark ? 'bg-[#242424]' : 'bg-white'}`}>
